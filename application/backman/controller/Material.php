...
class Material extends AuthBack{
    ...
    public function index(){
        ...
        foreach( $list as $key=>$item ){
            $list[$key]['image'] = photo_addpath($item['image']);
        }
    ...
    public function index_op($id = 0){
        if(!request()->isPost()){
            $info = Photo::get($id);
            $info['image'] = \photo_arr_path($info['image']);
            $list = Category::where('type','3')->select();
            $cateObj = new \lib\Category(['id','parent_id','name','cname']);
            $relist = $cateObj->getTree($list);
            $this->assign('group',$relist);
            ...
            $obj = new Photo();
            $this->data['image'] = \contentremotepath2($this->data['image']);
    ...
    public function delAll(){
        if(!request()->isAjax()){
            ...
            $id = $_POST['id'];
            AdminLog($this->admin['id'],'删除图片资料【'.implode(',',$id).'】');
            $state = Db::name('photo')->where('id','in',$id)->delete();
