...
class ApiController extends Controller{
    ...
    public $power = array();
    //权限验证跳过模块
    public $skinPow = ['Staff','Index','Upload'];
    ...
    private function checkPurview() {
        ...
        if( !in_array( $topKey, $this->skinPow ) ){

            if( !in_array( $topKey, $this->userPower['model'] ) ){
                return_ajax(40002,'没有权限!');
            }
     ...
     * 
     */
    private function setStaffPower(){
        $powerAll = DB::name('staff_power')->where('is_show',1)->select();
        $this->power = [];
        $userPower = DB::name('staff')->where('id',$this->userInfo['id'])->column('power');
        $userpower = explode( ',', $userPower[0] );
        foreach( $powerAll as $key=>$item ){
            if( !in_array( $item['model'], $this->power ) ){
                $this->power[] = $item['model'];
            }
            
            if( in_array( $item['id'], $userpower ) ){
                ...
                $arr['url'] = $item['url'];
                $arr['icon'] = photo_addpath($item['icon']);
                $arr['level'] = 1;
                if( empty( $this->userPower['model'] ) ){
                    $this->userPower['model'] = [];
                }
                

                if( !in_array( $item['model'], $this->userPower['model'] ) ){
                    $this->userPower['model'][] = $item['model'];
                }
                if( $item['parent_id'] != 0 ){
                    $arr['level'] = 2;
                    $this->userPower['powerStatus'][$item['model'].'_'.$item['id']] = true;
                }else{
                    $this->userPower['powerStatus'][$item['model']] = true;
                }
                
                // if( $item['parent_id'] == 0 ){
                    $this->userPower['power'][] = $arr;
                // }
                
            }else{
                if( $item['parent_id'] != 0 ){
                    $arr['level'] = 2;
                    $this->userPower['powerStatus'][$item['model'].'_'.$item['id']] = false;
                }else{
                    $this->userPower['powerStatus'][$item['model']] = false;
                }
            }
            

            if( empty( $this->userPower['action'] ) ){
                $this->userPower['action'] = [];
            }
            ...
            if( !empty($action) ){
                foreach( $action as $a ){
                    if( !in_array( $a, $this->userPower['action'] ) && !empty($a) && in_array( $item['id'], $userpower ) ){
                        
                        $this->userPower['action'][] = $item['model'].'/'.strtolower($a);
                    }
                }
            }
    ...
    public function orderInfoApi(){
        ...
        $info = OrderItem::where($where)->field('id,order_sn,cid,gid,did,spec,gsid,supplier,machine,composition,status,engding,remark,created_time')->find();
        
        $gsid = [];
        if( $info && $info['gsid'] ){
            $gsid = implode(',', json_decode( html_entity_decode($info['gsid']), true ) );
            $info['status'] = $info['status'] + 1;
            $info['gsid'] = json_html_decode($info['gsid']);
        }
